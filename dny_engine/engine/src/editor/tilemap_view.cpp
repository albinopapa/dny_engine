...
namespace dny{
	TileMapView::TileMapView( TileMap const& tilemap_, Rect<std::int32_t> const& area_ ) noexcept
		: m_tilemap{ tilemap_ }
		, m_area{ area_ }{
	}
	...
	void TileMapView::render( renderer2d& renderer, EditorCamera const& cam, std::unordered_map<std::string, surface<ColorF>> const& textures_ ) const{
		if( m_tilemap.size().width == 0 || m_tilemap.size().height == 0 ){
			return;
		}
		...
		auto max_tile_x = static_cast<std::int32_t>( std::floor( world_max.x / m_tile_size ) );
		auto min_tile_y = static_cast<std::int32_t>( std::floor( world_min.y / m_tile_size ) );
		auto max_tile_y = static_cast<std::int32_t>( std::floor( world_max.y / m_tile_size ) );

		min_tile_x = std::clamp( min_tile_x, 0, m_tilemap.size().width - 1 );
		max_tile_x = std::clamp( max_tile_x, 0, m_tilemap.size().width - 1 );
		min_tile_y = std::clamp( min_tile_y, 0, m_tilemap.size().height - 1 );
		max_tile_y = std::clamp( max_tile_y, 0, m_tilemap.size().height - 1 );
		...
		for( std::int32_t y = min_tile_y; y <= max_tile_y; ++y ){
			for( std::int32_t x = min_tile_x; x <= max_tile_x; ++x ){
				...
				const auto tile_rect = Rect<std::int32_t>{
					...
					static_cast< std::int32_t >( screen_pos.x + static_cast< float >( m_tile_size ) / cam.ortho_scale ),
					static_cast< std::int32_t >( screen_pos.y + static_cast< float >( m_tile_size ) / cam.ortho_scale )
				};
				...
				const auto& tile = m_tilemap.get_tile( tile_index );
				const auto& tile_def = g_tile_defs[ tile.definition_id ];
